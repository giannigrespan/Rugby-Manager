...
export const StaffMemberModal: React.FC<StaffMemberModalProps> = ({
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setIsAdmin(initialData.isAdmin ?? (initialData.role === 'programmatore' || initialData.role === 'head_coach'));
    ...
    try {
      if (initialData) {
        await onSave({
          ...
          role,
          isAdmin: isAdmin || role === 'programmatore',
          ...
          <div className="p-3.5 bg-[#1D1D21] border border-[#2A2A2E] rounded-xl space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className={`p-1.5 rounded-lg ${isAdmin || role === 'programmatore' ? 'bg-[#D4AF37]/20 text-[#D4AF37]' : 'bg-[#2A2A2E] text-gray-400'}`}>
                  <KeyRound className="w-4 h-4" />
                </div>
                <div>
                  <p className="text-xs font-bold text-white flex items-center gap-1.5">
                    <span>Permessi di Amministratore (Admin)</span>
                    {(isAdmin || role === 'programmatore') && (
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  checked={isAdmin || role === 'programmatore'}
                  onChange={(e) => setIsAdmin(e.target.checked)}
                  disabled={role === 'programmatore'}
